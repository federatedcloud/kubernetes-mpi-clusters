...
then
	...
	docker stop tf_kubernetes_container_gcp
	docker rm -f tf_kubernetes_container_gcp
	## Start the container, but just have it sleep so we can `docker exec` into it if necessary
	docker run --name tf_kubernetes_container_gcp $NIX_K8_TF_IMAGE sleep 20000 &
	sleep 5
	## Run terraform-kubernetes and copy out the results.
	docker exec -t tf_kubernetes_container_gcp nix-shell /home/nixuser/nix --run "/home/nixuser/nix/run.sh"
